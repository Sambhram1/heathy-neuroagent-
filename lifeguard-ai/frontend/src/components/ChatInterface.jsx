...
function buildSystemContext(userProfile, reportContext) {
  ...
  if (userProfile) {
    const bmi = userProfile.weight_kg && userProfile.height_cm
      ...
      : null
    parts.push(
      `User profile: Age ${userProfile.age}, ${userProfile.sex}, BMI ${bmi || 'unknown'}, ` +
      `Activity: ${userProfile.activity_level}, Sleep: ${userProfile.sleep_hours}h (${userProfile.sleep_quality}), ` +
      `Stress: ${userProfile.stress_level}/10, Smoking: ${userProfile.smoking ? 'yes' : 'no'}, ` +
      `Family history: ${userProfile.family_history?.join(', ') || 'none'}.`
    )
  }
  if (reportContext) {
    parts.push(`Uploaded diagnostic report: "${reportContext.filename}". ${reportContext.notes || ''}`)
  }
  return parts.join(' ')
}
...
export default function ChatInterface({ sessionId, onAgentResponse, planReady, onViewPlan, userProfile, reportContext }) {
  ...
  useEffect(() => {
    if (userProfile && !profileInjectedRef.current) {
      ...
      const ctx = buildSystemContext(userProfile, reportContext)
      if (ctx) {
        // Add a visible context note in the chat
        setMessages((prev) => [
          ...prev,
          {
            role: 'assistant',
            content: `Assessment profile loaded. I have your health parameters on record — ${ctx.split('.')[0]}. Ask me anything about your risks, recommendations, or what your scores mean.`,
          },
        ])
      }
    }
  }, [userProfile, reportContext])
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      // Build messages with profile context injected as system prefix
      const profileCtx = buildSystemContext(userProfile, reportContext)
      const apiMessages = profileCtx
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: `Connection sequence failed. Verify Neural API bridge (port 8000). Error: ${err.message}` },
      ])
    } finally {
      setIsLoading(false)
      setTimeout(() => inputRef.current?.focus(), 100)
    }
  }, [input, isLoading, messages, sessionId, onAgentResponse, userProfile, reportContext])
